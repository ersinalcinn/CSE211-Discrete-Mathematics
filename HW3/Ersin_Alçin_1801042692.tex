...
\end{figure*}
\solution 
\newline Given the graph, we examine the nodes in the graph, we give 1 for the value of that region of the node if there is a connection between the two nodes and this direction is in the right direction.For example, there is a connection from node A to node d, and since node a-b is shaped, the value in the matrix Table is 1.But since there is no connection from node A to node e, its value must be 0.
\newline
\hspace{2mm} a &\hspace{3mm} b &\hspace{2mm} c &\hspace{2mm} d &\hspace{2mm} e
\newline 
\begin{bmatrix}
\\0 & 1 & 0 & 1 & 0\\
\\1 & 0 & 1 & 1 & 1\\
...
\newline CSE101 is not linked to all classes except CSE346, we give these two lessons blue.
\newline CSE 102 is linked to all classes except CSE 346, so CSE 102 has a unique yellow color.
\newline Period 1:  MATH101, MATH 243 and CSE 211
\newline Period 2:  CSE 333
\newline Period 3:  CSE 273
\newline Period 4:  CSE 346 and CSE 101
\newline Period 5:  CSE 102
\begin{figure*}[htb]
	\centering
